...
class Funcionario(models.Model):
    ...
    DEPARTAMENTOS_CHOICES = [('TI', 'Tecnologia da Informação'), ('RH', 'Recursos Humanos'), ('FIN', 'Financeiro'), ('ADM', 'Administrativo'), ('COM', 'Comercial'), ('LOG', 'Logística'), ('JUR', 'Jurídico'), ('MKT', 'Marketing')]
    ...
    def calcular_inss(self):
        ...
        if salario > Decimal('7786.02'): return Decimal('908.85')
        ...
        if salario <= faixa1: return salario * Decimal('0.075')
        desc += faixa1 * Decimal('0.075')
        
        if salario <= faixa2: return desc + (salario - faixa1) * Decimal('0.09')
        desc += (faixa2 - faixa1) * Decimal('0.09')
        
        if salario <= faixa3: return desc + (salario - faixa2) * Decimal('0.12')
        desc += (faixa3 - faixa2) * Decimal('0.12')
        
        return desc + (salario - faixa3) * Decimal('0.14')
    ...
    @property
    def total_descontos(self): # Alias
